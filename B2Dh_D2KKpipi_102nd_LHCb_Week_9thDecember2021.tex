...
\institute{\normalsize University of Oxford \\ \vspace{0.3cm} \normalsize 102nd LHCb week \vspace{-0.2cm}}
...
\begin{frame}{Binned measurement of $\gamma$}
  \begin{itemize}
    ...
    \begin{itemize}
      \item{Non-optimal binning reduces statistical sensitivity}
      \item{But no bias is induced in final result}
    \end{itemize}
    \item{Need strong phases of $D$ decay $\to$ Will be measured at BESIII}
    \item{Analogous approach to $B^\pm\to Dh^\pm$, $D\to K_S^0 h^+h^-$}
    \begin{itemize}
      \item{\href{https://arxiv.org/abs/2010.08483}{JHEP \textbf{02} (2021) 0169}}
...
\begin{frame}{The BPGGSZ method}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      \item{$F_i = \frac{\int_i\dd{\Phi}\eta(\Phi)|\mathcal{A}(D^0)|^2}{\sum_j\int_j\dd{\Phi}\eta(\Phi)\abs{\mathcal{A}(D^0)}^2}$}
...
\begin{frame}{$\gamma$ precision benchmark}
  \begin{itemize}
    ...
    \vspace{-0.3cm}
    \begin{itemize}
      \item{Input values: $\gamma = \SI{75}{\degree}$, $\delta_B = \SI{130}{\degree}$, $r_B = 0.1$}
    \end{itemize}
    \item{Perform unbinned fit to the same amplitude model using AmpGen}
  \end{itemize}
  \begin{figure}
    \centering
    \vspace{-0.2cm}
...
\begin{frame}{Global mass fit}
  \begin{figure}
    ...
    \caption{Fit of $B^\pm\to DK^\pm$ channel (left) and $B^\pm\to D\pi^\pm$ channel (right) using full Run 1 and 2 data}
  \end{figure}
  \vspace{-0.5cm}
  \begin{itemize}
...
\begin{frame}{Binned CP fit}
  \begin{itemize}
    ...
    \item{$c_i$ and $s_i$ not available from BESIII yet $\implies$}
    \begin{itemize}
      \item{Calculate using MC integration of LHCb amplitude model for now}
    \end{itemize}
    \item{Use $2\times 8$ bins}
    \item{PDF shape parameters fixed from global mass fit}
...
\begin{frame}{CP observables result: $x_-^{DK}$ and $y_-^{DK}$}
  \begin{figure}
    \centering
    ...
    \begin{subfigure}{0.38\textwidth}
      ...
      \caption{$x_- = r_B\cos(\delta_B - \gamma)$}
    \end{subfigure}%
    \begin{subfigure}{0.38\textwidth}
      \includegraphics[width = 1.0\textwidth]{Plots/A_ym_dk_error_WithDataUncertainty.png}
      \caption{$y_- = r_B\sin(\delta_B - \gamma)$}
    \end{subfigure}
  \end{figure}
  \begin{center}
    Pulls and uncertainties from toy studies, uncertainty from data fit in blue
  \end{center}
\end{frame}
...
\begin{frame}{CP observables result: $x_\xi^{D\pi}$ and $y_\xi^{D\pi}$}
  \begin{figure}
    \centering
    \begin{subfigure}{0.38\textwidth}
...
\begin{frame}{$c_i$ and $s_i$ systematic uncertainty}
  \begin{itemize}
    ...
    \item{$c_i$ and $s_i$ uncertainty will come from BESIII analysis eventually}
    \begin{itemize}
...
\begin{frame}{Summary of all systematic uncertainties}
  ...
  \begin{tabular}{lcccccc} 
        ...
        \hline
        Statistical                              & 2.91  & 3.41  & 2.51  & 3.04  & 4.04  & 4.89  \\
        \hline
        ...
        \hline
        Total systematic                         & 0.77  & 1.55  & 0.41  & 1.34  & 2.01  & 1.23  \\
        \hline
  \end{tabular}
\end{frame}
...
\begin{frame}{Summary of CP observables}
  ...
  \end{itemize}
  \begin{align*}
    x_-^{DK} =& (x.x \pm 2.9 \pm 0.4 \pm 0.7)\times 10^{-2}, \\
    y_-^{DK} =& (x.x \pm 3.4 \pm 0.2 \pm 1.6)\times 10^{-2}, \\
    x_+^{DK} =& (x.x \pm 2.5 \pm 0.3 \pm 0.3)\times 10^{-2}, \\
    y_+^{DK} =& (x.x \pm 3.0 \pm 0.3 \pm 1.3)\times 10^{-2}, \\
    x_\xi^{D\pi} =& (x.x \pm 4.0 \pm 0.9 \pm 1.7)\times 10^{-2}, \\
    y_\xi^{D\pi} =& (x.x \pm 4.9 \pm 0.7 \pm 1.0)\times 10^{-2},
  \end{align*}
  \vspace{-0.5cm}
  \begin{itemize}
...
\section{Additional constraints from quasi-GLW observables}
\begin{frame}{Bonus measurement}
  \begin{itemize}
    ...
    \item{Quasi-GLW observables provide additional constraints on $\gamma$}
    \item{Measure observables for both $\pi^+\pi^-\pi^+\pi^-$ and $K^+K^-\pi^+\pi^-$}
  \end{itemize}
  \begin{align*}
    A_h =& \frac{\Gamma(B^-\to Dh^-) - \Gamma(B^+\to Dh^+)}{\Gamma(B^-\to Dh^-) + \Gamma(B^+\to Dh^+)}, \\
    R_{\rm CP} =& \frac{R(4h)}{R(K3\pi)}, \\
...
\begin{frame}{Global mass fit of $B^\pm\to Dh^\pm$, $D\to\pi^+\pi^-\pi^+\pi^-$}
  \vspace{-0.3cm}
  \begin{figure}
    \centering
  ...
  \vspace{-0.5cm}
  \begin{itemize}
    \item{Total yields are consistent with expectation given $KK\pi\pi$ yields}
  \end{itemize}
\end{frame}
...
\begin{frame}{Conclusion}
  \begin{itemize}
    ...
    \begin{itemize}
      \item{Expect total uncertainty around $\SI{12}{\degree}$-$\SI{14}{\degree}$}
    \end{itemize}
    ...
    \item{Analysis note: \href{https://twiki.cern.ch/twiki/pub/LHCbPhysics/GGSZB2DhD2hhpipiModelIndependent/LHCb-ANA-2021-051-v0.pdf}{LHCb-ANA-2021-051}}
  \end{itemize}
\end{frame}
